    ...
    public class JobEntityValidator : AbstractValidator<JobEntity>
    {
        public JobEntityValidator()
        {
            ...
            RuleFor(job => job.JobType)
                ...
                .WithMessage("Job type must be one of the following: FullTime, PartTime, Contract, Remote, Temporary.");

           
            RuleFor(job => job.JobAddress)
                .NotNull().WithMessage("Job address is required.");

          
            RuleFor(job => job.Vacancies)
                .NotNull().WithMessage("Number of vacancies is required.")
                .GreaterThanOrEqualTo(1).WithMessage("Vacancies must be at least 1.");

            RuleFor(job => job.BasicPay)
                .NotNull().WithMessage("Basic pay is required.");

            RuleFor(job => job.RequiredExperience)
                .NotNull().WithMessage("Required experience is mandatory.");

            RuleFor(job => job.PostingDate)
                .NotNull().WithMessage("Posting date is required.")
                .LessThanOrEqualTo(DateTime.UtcNow).WithMessage("Posting date cannot be in the future.");

            RuleFor(job => job.ApplicationDeadline)
                .NotNull().WithMessage("Application deadline is required.")
            ...
            RuleFor(job => job.FunctionalArea)
                .NotEmpty().WithMessage("Functional area is required.");
            ...
            RuleFor(job => job.JobResponsibilities)
                .NotEmpty().WithMessage("Job responsibilities must be provided.")
                .When(job => job.JobResponsibilities != null);

            RuleFor(job => job.Benefits)
                .NotEmpty().WithMessage("Benefits list cannot be empty.")
                .When(job => job.Benefits != null);

            RuleFor(job => job.PreferredQualifications)
                .NotEmpty().WithMessage("Preferred qualifications cannot be empty.")
