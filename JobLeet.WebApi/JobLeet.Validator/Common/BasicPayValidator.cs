    ...
    public class BasicPayValidator : AbstractValidator<BasicPay>
    {
        public BasicPayValidator()
        {
            RuleFor(basicPay => basicPay.MinmumPay)
                .NotNull().WithMessage("Minimum Pay is required.")
                .GreaterThanOrEqualTo(0).WithMessage("Minimum Pay cannot be negative.");

            RuleFor(basicPay => basicPay.MaximumPay)
                .NotNull().WithMessage("Maximum Pay is required.")
            ...
            RuleFor(basicPay => basicPay.Currency)
                ...
                .Must(currency => currency?.Length == 3).WithMessage("Currency must be a valid 3-character ISO code.")
